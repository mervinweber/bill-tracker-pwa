...
export function renderDebtSnowballView({ bills, settings }, actions) {
    ...
    const plan = buildDebtSnowballPlan(bills, extraPayment);
        ...
        <section class="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 sm:p-6" aria-label="Debt snowball planner">
            <div class="rounded-2xl border bg-card p-6 shadow-sm">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <h2 class="text-2xl font-semibold text-card-foreground">Debt Snowball Planner</h2>
                        <p class="mt-2 text-sm text-muted-foreground">Smallest balance first. Anything with an interest rate, debt total, or debt-snowball flag is included here.</p>
                    </div>
                    <form id="debtSnowballSettingsForm" class="grid gap-2 sm:grid-cols-[minmax(180px,220px)_auto]">
                        <label class="grid gap-1 text-sm">
                            <span class="text-xs text-muted-foreground">Extra Monthly Snowball Payment</span>
                        ...
                        <button type="submit" class="inline-flex h-10 items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">Save Extra Payment</button>
                    </form>
                </div>
    ...
    container.querySelector('#debtSnowballSettingsForm')?.addEventListener('submit', (event) => {
        ...
        actions.onSaveExtraPayment?.(amount);
    });
